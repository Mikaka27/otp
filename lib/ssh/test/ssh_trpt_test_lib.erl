...
-define(is_first_kex_dh(M),
        (M == ssh_msg_kexdh_init orelse
         M == ssh_msg_kexdh_init_guess)).
-define(is_first_kex_ecdh(M),
        (M == ssh_msg_kex_ecdh_init orelse
         M == ssh_msg_kex_ecdh_init_guess)).
-define(is_first_kex_msg(M),
        (M == ssh_msg_kexdh_init orelse
         M == ssh_msg_kex_ecdh_init)).
-define(is_first_kex_msg_guess(M),
        (M == ssh_msg_kexdh_init_guess orelse
         M == ssh_msg_kex_ecdh_init_guess)).
...
send(S0=#s{alg_neg={OwnMsg, PeerMsg}}, MsgType) when ?role(S0) == client,
                                                     ?is_first_kex_msg_guess(MsgType) ->
...
handle_first_kex_msg_to_send(MsgType, PeerMsg, OwnMsg, S0) ->
    ...
    S = opt(print_messages, S0,
            fun(X) when X==true;X==detail ->
                    case MsgType of
                        Dh when ?is_first_kex_dh(Dh); Dh == ssh_msg_kexdh_init_dup ->
                            #ssh{keyex_key = {{_Private, Public}, {_G, _P}}} = C,
                            Msg = #ssh_msg_kexdh_init{e = Public},
                            {"Send (reconstructed)~n~s~n",[format_msg(Msg)]};
                        Ecdh when ?is_first_kex_ecdh(Ecdh) ->
...
maybe_reset_alg_neg_and_guess_sent(#s{ssh = {send_mac = none}} = S) ->
    S;
