...
-export([
    ...
    schema_merge_should_work_with_external_backend/1
]).
...
all() -> [
    schema_merge_should_work_with_external_backend
].
...
schema_merge_should_work_with_external_backend(Config) when is_list(Config) ->
    [N1, N2] = ?init(2, Config),
    Ext = proplists:get_value(default_properties, Config, ?BACKEND),
